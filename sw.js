...
var CACHE_VERSION = 1;
var CURRENT_CACHES = {font: 'font-cache-v' + CACHE_VERSION};
    ...
    return cache.match(event.request)
        .then(function(response) {
          if (response) {
            ...
            // and we can just return it. Note that in this example, only font
          ...
          // cache.put() later on.
          // Both fetch() and cache.put() "consume" the request, so we need to
          // make a copy.
          // (see https://fetch.spec.whatwg.org/#dom-request-clone)
          return fetch(event.request.clone()).then(function(response) {
            console.log(
                '  Response for %s from network is: %O', event.request.url,
                response);
            ...
            if (response.status < 400 && response.headers.has('content-type') &&
              ...
              // i.e. have a Content-Type response header that starts with
              // "font/".
              ...
              // It is something to keep in mind if you're attempting to cache
              // other resources from a cross-origin
              // domain that doesn't support CORS, though!
              // We call .clone() on the response to save a copy of it to the
              // cache. By doing so, we get to keep
              // the original response object which we will return back to the
              // controlled page.
        ...
        .catch(function(error) {
          ...
          // fetch() operations.
          // Note that a HTTP error response (e.g. 404) will NOT trigger an
          // exception.
